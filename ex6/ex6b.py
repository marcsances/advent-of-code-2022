...
def get_sizes(node: Node, sizes: List[int]) -> int:
    size = 0
    for child in node.children.values():
        if child.type == NodeType.DIRECTORY:
            size = size + get_sizes(child, sizes)
        else:
...
def main():
    ...
    with open("input.txt", "r") as f:
        ...
        while inp is not None:
            ...
            inp = next(gen)
        root_size = get_sizes(root_node, sizes)
        print(sorted(filter(lambda size: size > 30000000 - (70000000 - root_size), sizes))[0])
